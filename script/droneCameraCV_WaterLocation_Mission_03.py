...
def Global_GPS_Position_CallBack(global_GPS_position):
    global global_GPS_position_Latitude
    global global_GPS_position_Longitude
    global global_GPS_position_Altitude
    ...
    print('I am priting the GPS coordinates')
    print(global_GPS_position_Latitude)
    print(global_GPS_position_Longitude)
    print(global_GPS_position_Altitude)
...
def Water_Reservoir_Discharge_Location_Identification_CallBack(img_msg):
    ...
    #The image coming from ROS which has been converted into OpenCV bu using 'bridge.imgmsg_to_cv2' will be 
    ...
    if len(cnts_red) > 0:
        ...
        if radius_red > 390 and lap_counter==1:

            #save the location of the Water_Reservoir on the parameter server
            rospy.set_param('/Water_Discharge_Location_Longitude',global_GPS_position_Longitude)
            rospy.set_param('/Water_Discharge_Location_Latitude',global_GPS_position_Latitude)
            rospy.set_param('/Water_Discharge_Location_Altitude',global_GPS_position_Altitude)
        #------------------------------------------------------------------------
    ...
    if len(cnts_blue) > 0:
        ...
        if radius_blue > 390 and lap_counter==1:

            #save the location of the Water_Reservoir on the parameter server
            rospy.set_param('/Water_Reservoir_Location_Longitude',global_GPS_position_Longitude)
            rospy.set_param('/Water_Reservoir_Location_Latitude',global_GPS_position_Latitude)
            rospy.set_param('/Water_Reservoir_Location_Altitude',global_GPS_position_Altitude)
        #------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    #---------------------------------------------------------------------------
    # 10th August 2022
    #I am defining a new Subscriber to get the global position of the drone
    #the topic is /mavros/global_position/global

    global_GPS_position_Subscriber=rospy.Subscriber('/mavros/global_position/global',NavSatFix, Global_GPS_Position_CallBack)
